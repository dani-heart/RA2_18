...
def lerTokens(caminho_arquivo: str) -> list[Token]:
    ...
    tokens = [] #Inicializa vazio e vai incrementando
    
    try:#Tenta abrir o arquivo
        with open(caminho_arquivo, 'r', encoding='utf-8') as f:
            for num_linha, linha_texto in enumerate(f, start=1):
                ...
                # O coração do nosso Lexer! Esta Regex separa os tokens da linha.
                # Regex: Separa os tokens de linha
                #Procura "(" ou ")" 
                #Ou caracteres que Não seja espaço ou parentesis
                partes = re.findall(r'\(|\)|[^\s()]+', linha_limpa)
                #E para cada parte manda para a função classificadora detalhada acima
                for parte in partes:
